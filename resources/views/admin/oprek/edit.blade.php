...
@section('title', 'Edit Form Rekrutmen')
@section('breadcrumb1', 'Manajemen')
@section('breadcrumb2', 'Edit Form Rekrutmen')
        ...
        <div class="row justify-content-md-center">
            <div class="col-xl-10">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('oprek.update', $form->id) }}" method="POST">
                            @csrf
                            @method('PUT')

                            <div class="form-group">
                                <label>Judul Form</label>
                                <input type="text" name="judul" class="form-control" value="{{ $form->judul }}" required>
                            </div>

                            <div class="form-group">
                                <label>Deskripsi</label>
                                <textarea name="deskripsi" class="form-control" required>{{ $form->deskripsi }}</textarea>
                            </div>

                            <div class="form-group">
                                <label>Batas Akhir</label>
                                <input type="datetime-local" name="batas_akhir" class="form-control" value="{{ \Carbon\Carbon::parse($form->batas_akhir)->format('Y-m-d\TH:i') }}" required>
                            </div>
                                    ...
                                    <div class="field-item row mb-2">
                                        ...
                                        <div class="col-md-2">
                                            <select name="fields[{{ $index }}][type]" class="form-control" required>
                                                @foreach(['text', 'number', 'email', 'date', 'textarea', 'file'] as $type)
                                                    <option value="{{ $type }}" {{ $field->type == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-2">
                                            <select name="fields[{{ $index }}][required]" class="form-control" required>
